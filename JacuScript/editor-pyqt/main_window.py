...
import jacu_lexer
from jacu_parser import *

class MainWindow(QMainWindow):

    def __init__(self):
        super(MainWindow, self).__init__()
        self.setWindowTitle(self.tr("JacuIDE"))
        ...
        self.run.triggered.connect(self._run)
    ...
    def __crear_acciones(self):
        ...
        self.run.setIcon(QIcon("imagenes/run.png"))
    ...
    def closeEvent(self, evento):
        if self.editor.modificado:
            ...
            r = QMessageBox.information(self, self.tr("Modificado!"),
                                self.tr("Desea guardar los cambios realizados?"),
                                flags)
            if r == QMessageBox.Yes:
                self._guardar_archivo()
            elif r == QMessageBox.No:
    ...
    def _run(self):
		contenido = str(self.editor.toPlainText())
		
		#jax = jacu_lexer.lexer.input(contenido)
		#archivo = '/media/bart/67369746-beb9-419b-965a-07a86b1209c0/home/bart/MEGAsync/JacuScript/editor-pyqt/Tabla.jacu'
		#TablaSimbolos = jacu_lexer.tokenize(contenido, jax)
		#TabladeSimbolos = jacu_lexer.inptokenize(contenido)
		#tabla = open(archivo, 'w')
		#tabla.write(TablaSimbolos)
		#jacu_lexer.test(contenido, jacu_lexer.lexer)
		parser.parse(contenido, tracking=True)
        
''''
    def run(self):
        if self.editor.es_nuevo:
			nombre = QFileDialog.getSaveFileName(self, self.tr("Cree el archivo Jacu antes de compilar"))
			if nombre:
				contenido = self.editor.toPlainText()
				with open(nombre, 'w') as archivo:
					archivo.write(contenido)
